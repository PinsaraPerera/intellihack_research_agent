...
def get_all(user_id: int, db: Session):
    configurations = (
        db.query(agent.Agent_Configurations)
        .filter(agent.Agent_Configurations.user_id == user_id)
        .all()
    )
...
def create(request: agentConfiguration_schema.ConfigurationCreate, db: Session):
    existing_project = (
        db.query(agent.Agent_Configurations)
        .filter(agent.Agent_Configurations.project_name == request.project_name)
        .first()
    )
...
def update_configuration(
    projectName: str,
    request: agentConfiguration_schema.ConfigurationCreate,
    db: Session,
):
    configuration = (
        db.query(agent.Agent_Configurations)
        .filter(agent.Agent_Configurations.project_name == projectName)
        .first()
    )
    if not configuration:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Configuration with project name {projectName} not found",
        )
...
def delete_configuration(projectName: str, db: Session):
    ...
    db.commit()
    return {
        "message": f"Configuration with project name {projectName} deleted successfully"
    }
...
def execute_configuration(projectName: str, db: Session):
    ...
    return agentConfiguration_schema.CustomResponse(
        response=response, date_created=datetime.now()
    )


def get_projects(user_id: int, db: Session):
    project_names = (
        db.query(agent.Agent_Configurations.project_name)
        .filter(agent.Agent_Configurations.user_id == user_id)
        .all()
    )
    ...
    if not project_names:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No projects found for user with id {user_id}",
        )
    
    names = [name[0] for name in project_names]

    response = {"projects": names, "date_created": datetime.now()}
    return response
